...
func NewBot(token string, debugChannelId int64) *Bot {
...
func (b *Bot) ReSend(c tgbotapi.Chattable) tgbotapi.Message {
	...
	if err != nil {
		...
		if botError.RetryAfter > 0 {
			...
			return b.ReSend(c)
		} else if botError.Code == 400 && b.DebugChannelId != 0 {
			var respRetry, err2 = b.Send(c)
			if err2 != nil {
				b.BotAPI.Send(tgbotapi.NewMessage(b.DebugChannelId, botError.Message))
			}
			return respRetry
		} else {
